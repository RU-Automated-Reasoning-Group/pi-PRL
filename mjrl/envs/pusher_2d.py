...
class Pusher2dEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self,
                 xml_file='pusher_2d.xml',
                 goal=(0, -1),
                 arm_object_distance_cost_coeff=0.5,  # previously, 0.0
    ...
    def compute_reward(self, observations, actions):
        ...
        distance = 0.0 if goal_object_distances < 0.07 else goal_object_distances - 0.07
        progress = 100 * (1 - max((distance - 0.07), 0.0) / (1 - 0.07))
