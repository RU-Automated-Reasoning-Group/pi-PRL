...
class TRPO(NPG):
    ...
    def train_from_paths(self, paths):
        ...
        # pick an arbitrary path's env_infos and check the keys
        env_infos = paths[0]['env_infos']
        if 'distance' in env_infos.keys() and 'progress' in env_infos.keys():
            avg_final_distance = np.mean([p['env_infos']['distance'][-1] for p in paths])
            avg_percentage = np.mean([p['env_infos']['progress'][-1] for p in paths])
            
            success_info = [p['env_infos']['finished'][-1] for p in paths]
            avg_success_rate = np.count_nonzero(success_info) / len(success_info)

            self.logger.log_kv('avg_final_distance_trpo', avg_final_distance)
            self.logger.log_kv('avg_progress_trpo', avg_percentage)
            base_stats += [avg_final_distance, avg_percentage, avg_success_rate]

            std_final_distance = np.std([p['env_infos']['distance'][-1] for p in paths])
            self.logger.log_kv('std_final_distance_trpo', std_final_distance)
        ...
        for k in range(100):
            ...
            if kl_dist < self.kl_dist:
                ...
                print(k, "Step size too high. Backtracking. | kl = %f | surr diff = %f" % \
